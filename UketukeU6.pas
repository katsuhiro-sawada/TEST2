...
uses
  ...
  Vcl.WinXPickers, Vcl.Grids, Vcl.Outline;


type
  TfrmUketuke6 = class(TForm)
    ds1: TDataSource;
    gb2: TGroupBox;
    Panel2: TPanel;
    btnNew: TBitBtn;
    ...
    ADOKoumoku: TADOQuery;
    DsKoumoku: TDataSource;
    ADOQuery1: TADOQuery;
    DBEdit16: TDBEdit;
    ADOTuikakoumoku: TADOQuery;
    ADOkingaku: TADOQuery;
    Button2: TButton;
    Label9: TLabel;
    Label21: TLabel;
    DBEdit17: TDBEdit;
...
//    procedure DateTimePicker4Change(Sender: TObject);
    ...
    procedure PtuikakoumokuClick(Sender: TObject);
...
//    procedure Button1Click(Sender: TObject);
  ...
  public
    ...
    type Bunsekikoumoku = record
      koumoku  : string ;   //項目
      CheckBox : TDBCheckBox ; // チェックボックス
    end;



var
  frmUketuke6: TfrmUketuke6;
    BunKou : array[1..15] of Bunsekikoumoku;


implementation

uses DMU,SHojouU, SUketukeU, MHojouU, BunsekiU;

{$R *.DFM}

procedure TfrmUketuke6.FormShow(Sender: TObject);
var
stmp : string;
mysqlstmp : String;
i,itmp : integer;
...
begin
  ...
  i := 1;
  BunKou[i].koumoku := 'pH';
  BunKou[i].CheckBox := DBCheckBox1;

  i := i + 1;
  BunKou[i].koumoku := 'EC';
  BunKou[i].CheckBox := DBCheckBox2;

  i := i + 1;
  BunKou[i].koumoku := 'CEC';
  BunKou[i].CheckBox := DBCheckBox3;

  i := i + 1;
  BunKou[i].koumoku := '石灰';
  BunKou[i].CheckBox := DBCheckBox4;

  i := i + 1;
  BunKou[i].koumoku := '苦土';
  BunKou[i].CheckBox := DBCheckBox5;

  i := i + 1;
  BunKou[i].koumoku := '加里';
  BunKou[i].CheckBox := DBCheckBox6;

  i := i + 1;
  BunKou[i].koumoku := 'リン酸';
  BunKou[i].CheckBox := DBCheckBox7;

  i := i + 1;
  BunKou[i].koumoku := '窒素';
  BunKou[i].CheckBox := DBCheckBox8;

  i := i + 1;
  BunKou[i].koumoku := 'ケイ酸';
  BunKou[i].CheckBox := DBCheckBox9;

  i := i + 1;
  BunKou[i].koumoku := '鉄';
  BunKou[i].CheckBox := DBCheckBox10;

  i := i + 1;
  BunKou[i].koumoku := '腐植';
  BunKou[i].CheckBox := DBCheckBox11;

  i := i + 1;
  BunKou[i].koumoku := 'ホウ素';
  BunKou[i].CheckBox := DBCheckBox12;

  i := i + 1;
  BunKou[i].koumoku := 'マンガン';
  BunKou[i].CheckBox := DBCheckBox13;

  i := i + 1;
  BunKou[i].koumoku := '銅';
  BunKou[i].CheckBox := DBCheckBox14;

  i := i + 1;
  BunKou[i].koumoku := '亜鉛';
  BunKou[i].CheckBox := DBCheckBox15;
...
//速度アップなら以下の作物関連のリンクテーブル処理を見直すと改善される
 ...
 // ADOZSakumokuConect.ConnectionString := mysqlstmp;
  ...
  ADOQSakumotubun.close;
  ADOQSakumotubun.Open;
...
procedure TfrmUketuke6.SetEditEnable(aFlag:Boolean);
begin
...
procedure TfrmUketuke6.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
...
procedure TfrmUketuke6.FormClose(Sender: TObject; var Action: TCloseAction);
begin
end;

{指定サンプル番号に移動}
procedure TfrmUketuke6.LocateNo( pSmplNo : string );
begin
  //編集中でなければサンプル番号に移動
	if ds1.DataSet.State in [dsEdit	,dsInsert] then exit;
    ADOUketuke.locate('受付連番',pSmplNo,[]);
end;
procedure TfrmUketuke6.PtuikakoumokuClick(Sender: TObject);
begin

end;

{圃場検索}
procedure TfrmUketuke6.btnSHojouClick(Sender: TObject);
begin
end;

//圃場番号変更
procedure TfrmUketuke6.txtHojouNoChange(Sender: TObject);
begin


end;
{新規ボタン}
procedure TfrmUketuke6.btnNewClick(Sender: TObject);
var
  bNewRec : Boolean ;
begin
    //最大サンプル番号割り当て
...
{閉じるボタン}
procedure TfrmUketuke6.btnCloseClick(Sender: TObject);
begin
...
{キャンセルボタン}
procedure TfrmUketuke6.btnCancelClick(Sender: TObject);
begin
	ds1.DataSet.Cancel;
end;
{登録ボタン}
procedure TfrmUketuke6.btnPostClick(Sender: TObject);
begin
  {入力チェック}
{
...
//新規の場合受付日セット
procedure TfrmUketuke6.Table1AfterInsert(DataSet: TDataSet);
begin
//	dtUketuke.Date := now ;
  {初期値の設定}
...
//更新前処理
procedure TfrmUketuke6.Table1BeforePost(DataSet: TDataSet);
begin
...
//更新後処理
procedure TfrmUketuke6.Table1AfterPost(DataSet: TDataSet);
begin
...
{削除ボタン}
procedure TfrmUketuke6.btnDelClick(Sender: TObject);
var
sint,eint : integer;
i : integer;
//stmp : string;
...
{編集ボタン}
procedure TfrmUketuke6.btnEditClick(Sender: TObject);
begin
	if ds1.DataSet.Bof and  ds1.DataSet.Eof then exit ;
	ds1.DataSet.Edit;
end;
{ステータス変更イベントで操作ボタンの制御}
procedure TfrmUketuke6.ds1StateChange(Sender: TObject);
begin
	//主キーは新規のみ修正可
...
procedure TfrmUketuke6.dssitenDataChange(Sender: TObject; Field: TField);
begin

end;

//検索
procedure TfrmUketuke6.btnSNoClick(Sender: TObject);
var
	SNO : string ;
begin
  SNO := DspUkeList(txtSampleNo.Text, '6');
  if SNO <> '' then begin
//  	Table1.Locate('サンプル番号',SNO,[]);
  	ADOUketuke.Locate('受付連番',SNO,[]);
  end;
end;
procedure TfrmUketuke6.Button1Click(Sender: TObject);
var
stmp : string;
itmp : integer;
count : integer;
...
if ds1.DataSet.FieldByName('作物分類').AsString = '' then begin

  showmessage('作物分類を入力して下さい。');
  exit;
end;
...
//受付NOを作成
  ...
  stmp := stmp + ds1.DataSet.FieldByName('作物分類').AsString;
...
procedure TfrmUketuke6.Button2Click(Sender: TObject);
begin


     Application.CreateForm(TfrmBunseki, frmBunseki);
     frmBunseki.EMode.Text := '6';
     frmBunseki.EUketukeNo.text := ds1.Dataset.FieldByName('受付NO').AsString;
...
procedure TfrmUketuke6.Table1AfterScroll(DataSet: TDataSet);
begin
...
procedure TfrmUketuke6.cmbJikiChange(Sender: TObject);
begin
end;
//DB非リンク項目編集されれば、自動エディットする。
procedure TfrmUketuke6.dtUketukeChange(Sender: TObject);
begin
...
//キャンセルしたら非リンク項目を戻す
procedure TfrmUketuke6.Table1AfterCancel(DataSet: TDataSet);
begin
...
{データ編集イベント}
procedure TfrmUketuke6.ds1DataChange(Sender: TObject; Field: TField);
var
stmp : string;
i : integer;
kingaku : integer;
...
begin
  ...
  ADOTuikakoumoku.close;

  stmp := 'select * from T_TUIKASEIBUN where (コード = ''' + ds1.DataSet.FieldByName('分析項目').asString +''') and ('+
  '作物種類 = ''' + ds1.DataSet.FieldByName('作物分類').asString + ''')';

  ADOTuikakoumoku.SQL.Text := stmp;

  ADOTuikakoumoku.open;


  kingaku := 0;
  itmp := 0;
  if ADOTuikakoumoku.RecordCount > 0 then begin


  //Ptuikakoumoku.Visible := true;


  ADOkingaku.Close;
  stmp := 'select * from M_KOUMOKU where コード = ''' + ds1.DataSet.FieldByName('分析項目').asString + '''';
  ADOkingaku.SQL.Text := stmp;
  ADOkingaku.Open;

  kingaku := ADOkingaku.fieldbyname('金額').asinteger;


  for i := 1 to 15 do begin

    if ADOTuikakoumoku.FieldByName(BunKou[i].koumoku).asString = '○' then begin
//      BunKou[i].checkbox.Checked := true;
      BunKou[i].checkbox.Enabled := false;
    end
    else if ADOTuikakoumoku.FieldByName(BunKou[i].koumoku).asString = '―' then begin
//      BunKou[i].checkbox.Checked := false;
      BunKou[i].checkbox.Enabled := false;
    end
    else begin

      BunKou[i].checkbox.Enabled := true;
      if       BunKou[i].checkbox.Checked = true then begin

        itmp := itmp + strtointdef( ADOTuikakoumoku.FieldByName(BunKou[i].koumoku).asString, 0);
      end;
    ...
    //Ptuikakoumoku.Visible := false;

  end;



  Lkingaku.Caption := inttostr(kingaku+itmp);
  LFmtText := ',0';
  Lkingaku.Caption := '\' + FormatFloat(LFmtText, Uint64(kingaku+itmp)) ;
...
{Enterキーでタブ移動}
procedure TfrmUketuke6.EnterKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfrmUketuke6.BtnClrClick(Sender: TObject);
var
  aBuff : string ;
begin
...
procedure TfrmUketuke6.txtSampleNoExit(Sender: TObject);
var
  iNum : integer ;
begin
...
procedure TfrmUketuke6.ESakuCodeKeyPress(Sender: TObject; var Key: Char);
begin
 ESakuCodeFlg := true;
...
procedure TfrmUketuke6.cmbSakumokuDropDown(Sender: TObject);
begin
    ESakuCodeFlg := false;
    ESakuCodeFlg2 := true;
...
procedure TfrmUketuke6.cmbSakumotuDropDown(Sender: TObject);
begin
    ESakuCodeFlg := false;
    ESakuCodeFlg2 := true;
...
procedure TfrmUketuke6.cmbSakugataDropDown(Sender: TObject);
begin
    ESakuCodeFlg := false;
    ESakuCodeFlg2 := true;
...
procedure TfrmUketuke6.DateTimePicker1Change(Sender: TObject);
begin
//データ修正
  	if not (ds1.DataSet.State  in [dsEdit	,dsInsert]) then
    ds1.DataSet.Edit ;
...
procedure TfrmUketuke6.DateTimePicker2Change(Sender: TObject);
begin
...
procedure TfrmUketuke6.DateTimePicker3Change(Sender: TObject);
begin
...
procedure TfrmUketuke6.DateTimePicker4Change(Sender: TObject);
begin
//データ修正
  	if not (ds1.DataSet.State  in [dsEdit	,dsInsert]) then
    ds1.DataSet.Edit ;
...
procedure TfrmUketuke6.DateTimePicker5Change(Sender: TObject);
begin
  	if not (ds1.DataSet.State  in [dsEdit	,dsInsert]) then
    ds1.DataSet.Edit ;
...
procedure TfrmUketuke6.DateTimePicker6Change(Sender: TObject);
begin
  	if not (ds1.DataSet.State  in [dsEdit	,dsInsert]) then
    ds1.DataSet.Edit ;
...
procedure TfrmUketuke6.DateTimePicker7Change(Sender: TObject);
begin
  	if not (ds1.DataSet.State  in [dsEdit	,dsInsert]) then
    ds1.DataSet.Edit ;
...
procedure TfrmUketuke6.DBLookupComboBox1DropDown(Sender: TObject);
begin
...
procedure TfrmUketuke6.ADOUketukeAfterCancel(DataSet: TDataSet);
begin
  FNewNO := '';     //Post,Cancelした後はクリア {Version 0.6 #38}
...
procedure TfrmUketuke6.ADOUketukeAfterInsert(DataSet: TDataSet);
begin
  ds1.DataSet.FieldByName('編集日').asDatetime := now ;  //送信フラグの手前、手動で更新する
//	ds1.DataSet.FieldByName('削除フラグ').asBoolean := False ;
...
procedure TfrmUketuke6.ADOUketukeAfterPost(DataSet: TDataSet);
begin
  //もし、新規作成なら、診断テーブルを作成する。 {Version 0.6 #38}
//  if FNewNO <> '' then
...
procedure TfrmUketuke6.ADOUketukeAfterScroll(DataSet: TDataSet);
var
stmp : string;
i : integer;
begin
...
procedure TfrmUketuke6.ADOUketukeBeforePost(DataSet: TDataSet);
begin
