...
procedure TfrmUketuke3.FormShow(Sender: TObject);
var
stmp : string;
mysqlstmp : String;
i,itmp : integer;
...
procedure TfrmUketuke3.SetEditEnable(aFlag:Boolean);
begin
...
procedure TfrmUketuke3.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
...
procedure TfrmUketuke3.FormClose(Sender: TObject; var Action: TCloseAction);
begin
end;

{指定サンプル番号に移動}
procedure TfrmUketuke3.LocateNo( pSmplNo : string );
begin
  //編集中でなければサンプル番号に移動
	if ds1.DataSet.State in [dsEdit	,dsInsert] then exit;
    ADOUketuke.locate('受付連番',pSmplNo,[]);
end;
procedure TfrmUketuke3.Memo1Change(Sender: TObject);
begin

end;

{圃場検索}
procedure TfrmUketuke3.btnSHojouClick(Sender: TObject);
begin
end;

//圃場番号変更
procedure TfrmUketuke3.txtHojouNoChange(Sender: TObject);
begin


end;
{新規ボタン}
procedure TfrmUketuke3.btnNewClick(Sender: TObject);
var
  bNewRec : Boolean ;
begin
    //最大サンプル番号割り当て
...
{閉じるボタン}
procedure TfrmUketuke3.btnCloseClick(Sender: TObject);
begin
...
{キャンセルボタン}
procedure TfrmUketuke3.btnCancelClick(Sender: TObject);
begin
	ds1.DataSet.Cancel;
end;
{登録ボタン}
procedure TfrmUketuke3.btnPostClick(Sender: TObject);
begin
  {入力チェック}
{
...
//新規の場合受付日セット
procedure TfrmUketuke3.Table1AfterInsert(DataSet: TDataSet);
begin
//	dtUketuke.Date := now ;
  {初期値の設定}
...
//更新前処理
procedure TfrmUketuke3.Table1BeforePost(DataSet: TDataSet);
begin
...
//更新後処理
procedure TfrmUketuke3.Table1AfterPost(DataSet: TDataSet);
begin
...
{削除ボタン}
procedure TfrmUketuke3.btnDelClick(Sender: TObject);
var
sint,eint : integer;
i : integer;
//stmp : string;
...
{編集ボタン}
procedure TfrmUketuke3.btnEditClick(Sender: TObject);
begin
	if ds1.DataSet.Bof and  ds1.DataSet.Eof then exit ;
	ds1.DataSet.Edit;
end;
{ステータス変更イベントで操作ボタンの制御}
procedure TfrmUketuke3.ds1StateChange(Sender: TObject);
begin
	//主キーは新規のみ修正可
...
procedure TfrmUketuke3.dssitenDataChange(Sender: TObject; Field: TField);
begin

end;

//検索
procedure TfrmUketuke3.btnSNoClick(Sender: TObject);
var
	SNO : string ;
begin
...
procedure TfrmUketuke3.Button1Click(Sender: TObject);
var
stmp : string;
itmp : integer;
count : integer;
...
procedure TfrmUketuke3.Table1AfterScroll(DataSet: TDataSet);
begin
...
procedure TfrmUketuke3.cmbJikiChange(Sender: TObject);
begin
end;
//DB非リンク項目編集されれば、自動エディットする。
procedure TfrmUketuke3.dtUketukeChange(Sender: TObject);
begin
...
//キャンセルしたら非リンク項目を戻す
procedure TfrmUketuke3.Table1AfterCancel(DataSet: TDataSet);
begin
...
{データ編集イベント}
procedure TfrmUketuke3.ds1DataChange(Sender: TObject; Field: TField);
begin
...
{Enterキーでタブ移動}
procedure TfrmUketuke3.EnterKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfrmUketuke3.BtnClrClick(Sender: TObject);
var
  aBuff : string ;
begin
...
procedure TfrmUketuke3.txtSampleNoExit(Sender: TObject);
var
  iNum : integer ;
begin
...
procedure TfrmUketuke3.ESakuCodeKeyPress(Sender: TObject; var Key: Char);
begin
 ESakuCodeFlg := true;
...
procedure TfrmUketuke3.cmbSakumokuDropDown(Sender: TObject);
begin
    ESakuCodeFlg := false;
    ESakuCodeFlg2 := true;
...
procedure TfrmUketuke3.cmbSakumotuDropDown(Sender: TObject);
begin
    ESakuCodeFlg := false;
    ESakuCodeFlg2 := true;
...
procedure TfrmUketuke3.cmbSakugataDropDown(Sender: TObject);
begin
    ESakuCodeFlg := false;
    ESakuCodeFlg2 := true;
...
procedure TfrmUketuke3.DateTimePicker1Change(Sender: TObject);
begin
//データ修正
  	if not (ds1.DataSet.State  in [dsEdit	,dsInsert]) then
    ds1.DataSet.Edit ;
...
procedure TfrmUketuke3.DateTimePicker2Change(Sender: TObject);
begin
...
procedure TfrmUketuke3.DateTimePicker3Change(Sender: TObject);
begin
...
procedure TfrmUketuke3.DateTimePicker4Change(Sender: TObject);
begin
//データ修正
  	if not (ds1.DataSet.State  in [dsEdit	,dsInsert]) then
    ds1.DataSet.Edit ;
...
procedure TfrmUketuke3.DateTimePicker5Change(Sender: TObject);
begin
  	if not (ds1.DataSet.State  in [dsEdit	,dsInsert]) then
    ds1.DataSet.Edit ;
...
procedure TfrmUketuke3.DateTimePicker6Change(Sender: TObject);
begin
  	if not (ds1.DataSet.State  in [dsEdit	,dsInsert]) then
    ds1.DataSet.Edit ;
...
procedure TfrmUketuke3.DateTimePicker7Change(Sender: TObject);
begin
  	if not (ds1.DataSet.State  in [dsEdit	,dsInsert]) then
    ds1.DataSet.Edit ;
...
procedure TfrmUketuke3.DBLookupComboBox1DropDown(Sender: TObject);
begin
...
procedure TfrmUketuke3.ADOUketukeAfterCancel(DataSet: TDataSet);
begin
  FNewNO := '';     //Post,Cancelした後はクリア {Version 0.6 #38}
...
procedure TfrmUketuke3.ADOUketukeAfterInsert(DataSet: TDataSet);
begin
  ds1.DataSet.FieldByName('編集日').asDatetime := now ;  //送信フラグの手前、手動で更新する
//	ds1.DataSet.FieldByName('削除フラグ').asBoolean := False ;
...
procedure TfrmUketuke3.ADOUketukeAfterPost(DataSet: TDataSet);
begin
  //もし、新規作成なら、診断テーブルを作成する。 {Version 0.6 #38}
//  if FNewNO <> '' then
...
procedure TfrmUketuke3.ADOUketukeAfterScroll(DataSet: TDataSet);
begin
...
procedure TfrmUketuke3.ADOUketukeBeforePost(DataSet: TDataSet);
begin
