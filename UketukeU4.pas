...
procedure TfrmUketuke4.FormShow(Sender: TObject);
var
stmp : string;
mysqlstmp : String;
i,itmp : integer;
...
procedure TfrmUketuke4.SetEditEnable(aFlag:Boolean);
begin
...
procedure TfrmUketuke4.FormCloseQuery(Sender: TObject; var CanClose: Boolean);
begin
...
procedure TfrmUketuke4.FormClose(Sender: TObject; var Action: TCloseAction);
begin
end;

{指定サンプル番号に移動}
procedure TfrmUketuke4.LocateNo( pSmplNo : string );
begin
  //編集中でなければサンプル番号に移動
	if ds1.DataSet.State in [dsEdit	,dsInsert] then exit;
    ADOUketuke.locate('受付連番',pSmplNo,[]);
end;
procedure TfrmUketuke4.Memo1Change(Sender: TObject);
begin

end;

{圃場検索}
procedure TfrmUketuke4.btnSHojouClick(Sender: TObject);
begin
end;

//圃場番号変更
procedure TfrmUketuke4.txtHojouNoChange(Sender: TObject);
begin


end;
{新規ボタン}
procedure TfrmUketuke4.btnNewClick(Sender: TObject);
var
  bNewRec : Boolean ;
begin
    //最大サンプル番号割り当て
...
{閉じるボタン}
procedure TfrmUketuke4.btnCloseClick(Sender: TObject);
begin
...
{キャンセルボタン}
procedure TfrmUketuke4.btnCancelClick(Sender: TObject);
begin
	ds1.DataSet.Cancel;
end;
{登録ボタン}
procedure TfrmUketuke4.btnPostClick(Sender: TObject);
begin
  {入力チェック}
{
...
//新規の場合受付日セット
procedure TfrmUketuke4.Table1AfterInsert(DataSet: TDataSet);
begin
//	dtUketuke.Date := now ;
  {初期値の設定}
...
//更新前処理
procedure TfrmUketuke4.Table1BeforePost(DataSet: TDataSet);
begin
...
//更新後処理
procedure TfrmUketuke4.Table1AfterPost(DataSet: TDataSet);
begin
...
{削除ボタン}
procedure TfrmUketuke4.btnDelClick(Sender: TObject);
var
sint,eint : integer;
i : integer;
//stmp : string;
...
{編集ボタン}
procedure TfrmUketuke4.btnEditClick(Sender: TObject);
begin
	if ds1.DataSet.Bof and  ds1.DataSet.Eof then exit ;
	ds1.DataSet.Edit;
end;
{ステータス変更イベントで操作ボタンの制御}
procedure TfrmUketuke4.ds1StateChange(Sender: TObject);
begin
	//主キーは新規のみ修正可
...
procedure TfrmUketuke4.dssitenDataChange(Sender: TObject; Field: TField);
begin

end;

//検索
procedure TfrmUketuke4.btnSNoClick(Sender: TObject);
var
	SNO : string ;
begin
  SNO := DspUkeList(txtSampleNo.Text, '4');
  if SNO <> '' then begin
//  	Table1.Locate('サンプル番号',SNO,[]);
  	ADOUketuke.Locate('受付連番',SNO,[]);
  end;
end;
procedure TfrmUketuke4.Button1Click(Sender: TObject);
var
stmp : string;
itmp : integer;
count : integer;
...
procedure TfrmUketuke4.Table1AfterScroll(DataSet: TDataSet);
begin
...
procedure TfrmUketuke4.cmbJikiChange(Sender: TObject);
begin
end;
//DB非リンク項目編集されれば、自動エディットする。
procedure TfrmUketuke4.dtUketukeChange(Sender: TObject);
begin
...
//キャンセルしたら非リンク項目を戻す
procedure TfrmUketuke4.Table1AfterCancel(DataSet: TDataSet);
begin
...
{データ編集イベント}
procedure TfrmUketuke4.ds1DataChange(Sender: TObject; Field: TField);
begin
...
{Enterキーでタブ移動}
procedure TfrmUketuke4.EnterKeyPress(Sender: TObject; var Key: Char);
begin
...
procedure TfrmUketuke4.BtnClrClick(Sender: TObject);
var
  aBuff : string ;
begin
...
procedure TfrmUketuke4.txtSampleNoExit(Sender: TObject);
var
  iNum : integer ;
begin
...
procedure TfrmUketuke4.ESakuCodeKeyPress(Sender: TObject; var Key: Char);
begin
 ESakuCodeFlg := true;
...
procedure TfrmUketuke4.cmbSakumokuDropDown(Sender: TObject);
begin
    ESakuCodeFlg := false;
    ESakuCodeFlg2 := true;
...
procedure TfrmUketuke4.cmbSakumotuDropDown(Sender: TObject);
begin
    ESakuCodeFlg := false;
    ESakuCodeFlg2 := true;
...
procedure TfrmUketuke4.cmbSakugataDropDown(Sender: TObject);
begin
    ESakuCodeFlg := false;
    ESakuCodeFlg2 := true;
...
procedure TfrmUketuke4.DateTimePicker1Change(Sender: TObject);
begin
//データ修正
  	if not (ds1.DataSet.State  in [dsEdit	,dsInsert]) then
    ds1.DataSet.Edit ;
...
procedure TfrmUketuke4.DateTimePicker2Change(Sender: TObject);
begin
...
procedure TfrmUketuke4.DateTimePicker3Change(Sender: TObject);
begin
...
procedure TfrmUketuke4.DateTimePicker4Change(Sender: TObject);
begin
//データ修正
  	if not (ds1.DataSet.State  in [dsEdit	,dsInsert]) then
    ds1.DataSet.Edit ;
...
procedure TfrmUketuke4.DateTimePicker5Change(Sender: TObject);
begin
  	if not (ds1.DataSet.State  in [dsEdit	,dsInsert]) then
    ds1.DataSet.Edit ;
...
procedure TfrmUketuke4.DateTimePicker6Change(Sender: TObject);
begin
  	if not (ds1.DataSet.State  in [dsEdit	,dsInsert]) then
    ds1.DataSet.Edit ;
...
procedure TfrmUketuke4.DateTimePicker7Change(Sender: TObject);
begin
  	if not (ds1.DataSet.State  in [dsEdit	,dsInsert]) then
    ds1.DataSet.Edit ;
...
procedure TfrmUketuke4.DBLookupComboBox1DropDown(Sender: TObject);
begin
...
procedure TfrmUketuke4.ADOUketukeAfterCancel(DataSet: TDataSet);
begin
  FNewNO := '';     //Post,Cancelした後はクリア {Version 0.6 #38}
...
procedure TfrmUketuke4.ADOUketukeAfterInsert(DataSet: TDataSet);
begin
  ds1.DataSet.FieldByName('編集日').asDatetime := now ;  //送信フラグの手前、手動で更新する
//	ds1.DataSet.FieldByName('削除フラグ').asBoolean := False ;
...
procedure TfrmUketuke4.ADOUketukeAfterPost(DataSet: TDataSet);
begin
  //もし、新規作成なら、診断テーブルを作成する。 {Version 0.6 #38}
//  if FNewNO <> '' then
...
procedure TfrmUketuke4.ADOUketukeAfterScroll(DataSet: TDataSet);
begin
...
procedure TfrmUketuke4.ADOUketukeBeforePost(DataSet: TDataSet);
begin
